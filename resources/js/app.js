...
Vue.filter('userAddress', function(user){
    var address = '';
    if(user == null){ return address;}
    else {
        address = user.customer_address ? user.customer_address.street+', ' : '';
        address = address+' '+(user.customer_address && user.customer_address.street2 ? user.customer_address.street2+', ': '');
        address = address+' '+(user.customer_address ?  user.customer_address.city+',' : '');
        address = address+' '+(user.customer_address && user.customer_address.state_id ? user.customer_address.state.name: '');
        
        return address;
    }
})

Vue.filter('userImage', function(user){
    var picture = '';
    if(user == null){ return '/img/profile/default.png';}
    else {return '/img/profile/'+user.image ;}
})
