    ...
    <table class="table table-hover text-nowrap" v-if="unverified_bvns.data != null && unverified_bvns.data.length != 0">
        ...
        <tbody>
            <tr v-for="bvn in unverified_bvns.data" :key="bvn.id">
                <td>{{ bvn | FullName  }}</td>
                <td>{{ bvn.bvn}}</td>
                <td>{{ bvn.updated_at | excelDate }}</td>
                <td>{{ specific | firstUp }}</td>
                <td>
                    <button type="button" class="btn btn-light" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-ellipsis-v"></i></button>
                    <div class="dropdown-menu">
                        <router-link class="btn btn-block dropdown-item" :to="'/staff/confirm/guarantor_bvn/'+bvn.id"><i class="fa fa-check mr-1 text-primary"></i> Confirm </router-link>
                    </div>
                </td>
            </tr>
        </tbody>
...
export default {
    ...
    methods:{
        getAllInitials(page = 1){
            axios.get('/api/ums/bvn_verifications?page='+page+'&point=guarantor&specific='+this.specific)
            .then(response =>{
                this.unverified_bvns = response.data.unverified_bvns;
            })
            .catch(()=>{
                this.$Progress.fail();
                toast.fire({icon: 'error', title: 'Guarantor Banks Verification Numbers not loaded successfully',});
            });
        },
    },
    mounted() {
